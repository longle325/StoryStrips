...
import { loadCachedComics, removeComicFromCache, type CachedComic } from "@/lib/comicCache";

const MyStoriesTab = () => {
  const [stories, setStories] = useState<CachedComic[]>([]);

  useEffect(() => {
    setStories(loadCachedComics());
  }, []);
  ...
  const handleDelete = (comicId: string) => {
    const next = removeComicFromCache(comicId);
    setStories(next);
  };
        ...
        {sortedStories.map((story, i) => (
          ...
          >
            <div className="relative h-44">
              {story.panels[0]?.image_url ? (
                <img src={story.panels[0].image_url} alt={story.script.title} className="h-full w-full object-cover" />
              ) : (
              ...
              <div className="absolute inset-0 flex items-center justify-center gap-2 bg-background/60 opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100">
                <button
                  type="button"
                  onClick={() => handleDelete(story.comic_id)}
            ...
            <div className="p-3">
              <p className="line-clamp-1 text-sm font-medium text-foreground">{story.script.title}</p>
              <div className="mt-1 flex items-center justify-between">
                <span className="text-xs text-muted-foreground">
                  {new Date(story.created_at).toLocaleDateString("en-GB")}
                </span>
                <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">
                  {story.script.art_style}
                </span>
              </div>
            </div>
          </motion.div>
